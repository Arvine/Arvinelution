...
function c5665.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc~=c and c5665.atkfilter(chkc,c:GetAttack()) end
	if chk==0 then return Duel.IsExistingTarget(c5665.atkfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,c,c:GetAttack()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local g=Duel.SelectTarget(tp,c5665.atkfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,c,c:GetAttack())
end
