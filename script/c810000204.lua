...
function c810000204.initial_effect(c)
	...
	e3:SetOperation(c810000204.desop)
	c:RegisterEffect(e3)
end
function c810000204.filter1(c,e)
	return c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e)
end
function c810000204.filter2(c,e,tp,m,chkf)
...
function c810000204.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg1=Duel.GetMatchingGroup(c810000204.filter1,tp,LOCATION_DECK,0,nil,e)
		local res=Duel.IsExistingMatchingCard(c810000204.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,chkf)
		if not res then
			local ce=Duel.GetChainMaterial(tp)
			if ce~=nil then
				local fgroup=ce:GetTarget()
				local mg2=fgroup(ce,e,tp)
				res=Duel.IsExistingMatchingCard(c810000204.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,chkf)
			end
		end
		return res
	end
...
function c810000204.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
	local mg1=Duel.GetMatchingGroup(c810000204.filter1,tp,LOCATION_DECK,0,nil,e)
	local sg1=Duel.GetMatchingGroup(c810000204.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		local fgroup=ce:GetTarget()
		mg2=fgroup(ce,e,tp)
		sg2=Duel.GetMatchingGroup(c810000204.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,chkf)
	end
