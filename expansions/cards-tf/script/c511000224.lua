--Destiny Activator
function c511000224.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DECKDES)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetOperation(c511000224.activate)
	c:RegisterEffect(e1)
end
function c511000224.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct>0 then
		...
		if tcs:IsType(TYPE_MONSTER) then
			...
			e1:SetOperation(c511000224.op1)
		...
		elseif tcs:IsType(TYPE_SPELL) then
			...
			e1:SetCode(EVENT_DRAW)
			e1:SetRange(LOCATION_SZONE)
			e1:SetOperation(c511000224.op2)
			e1:SetReset(RESET_PHASE+PHASE_END+RESET_SELF_TURN,3)
			c:RegisterEffect(e1)
		else
			local e1=Effect.CreateEffect(c)
			...
			e1:SetCode(EVENT_DRAW)
			e1:SetRange(LOCATION_SZONE)
			e1:SetOperation(c511000224.op3)
			e1:SetReset(RESET_PHASE+PHASE_END+RESET_SELF_TURN,3)
			c:RegisterEffect(e1)
		end
		...
		e2:SetLabel(3)
		e2:SetCountLimit(1)
		e2:SetOperation(c511000224.tgop)
		e2:SetReset(RESET_PHASE+PHASE_END+RESET_SELF_TURN,3)
		c:RegisterEffect(e2)
	end
end
function c511000224.op1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c511000224.op2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if ep==tp and c:IsLocation(LOCATION_SZONE) and c:IsFaceup() then return end
	local hg=eg:Filter(Card.IsLocation,nil,LOCATION_HAND)
	if hg:GetCount()==0 then return end
...
function c511000224.op3(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if ep==tp and c:IsLocation(LOCATION_SZONE) and c:IsFaceup() then return end
	local hg=eg:Filter(Card.IsLocation,nil,LOCATION_HAND)
	if hg:GetCount()==0 then return end
...
function c511000224.tgop(e,tp,eg,ep,ev,re,r,rp)
