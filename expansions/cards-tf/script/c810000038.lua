...
function c810000038.spfilter(c,e,tp)
	return c:IsCode(810000037) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c810000038.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c810000038.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c810000038.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c810000038.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
